...
class SubCategoryController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        $rules = [
            'name'                 => 'required',
            'name_ar'              => 'required',
            'slug'                 => 'unique:subcategories,slug,' . $id . '',
            'category_id'          => 'required'
        ];
        $messages = [
            'name.required'                                      => trans('Name') .  '  ' . trans('required'),
        ...
        if ($file = $request->file('photo')) {
            ...
            $file->move('public/assets/images/subcategories', $name);
            if ($data->photo != null) {
                if (file_exists(public_path() . '/public/assets/images/subcategories/' . $data->photo)) {
                    unlink(public_path() . '/public/assets/images/subcategories/' . $data->photo);
                }
            }
            $input['photo'] = $name;
        }
        ...
        if (!empty($request->input('selected_products_delete'))) {
            ...
            foreach ($products as $data) {
                ...
                if ($data->childs->count() > 0) {
                    ...
                    Session::flash('error', 'Remove the childcategory first !');
                    return Redirect()->back();
                    //--- Redirect Section Ends
                }
                if ($data->products->count() > 0) {
