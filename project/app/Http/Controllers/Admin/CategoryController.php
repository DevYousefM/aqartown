...
class CategoryController extends Controller
{
    ...
    public function datatables()
    {
        ...
        return Datatables::of($datas)
            ...
            ->toJson(); //--- Returning Json Data To Client Side  <a href="javascript" class="set-gallery" data-toggle="modal" data-target="#setgallery"><input type="hidden" value="'.$data->id.'"><i class="fas fa-eye"></i> View Gallery</a>
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->is_featured == '') {
            ...
            //--- Validation Section
            $rules = [
                'image' => 'mimes:jpeg,webp,jpg,png,svg',
            ];
            $customs = [
    ...
    public function ckeckall(Request $request)
    {
        if (!empty($request->input('selected_products'))) {
            ...
            $products = Category::whereIn('id', $selected_products)->update(['status' => 0]);
    ...
    public function ckeckactivate(Request $request)
    {
        if (!empty($request->input('selected_products_activate'))) {
            ...
            $products = Category::whereIn('id', $selected_products)->update(['status' => 1]);
    ...
    public function ckeckdelete(Request $request)
    {
        if (!empty($request->input('selected_products_delete'))) {
            ...
            $products = Category::whereIn('id', $selected_products)->get();
            ...
            foreach ($products as $data) {
                ...
                if ($data->products->count() > 0) {
                    ...
                    return Redirect()
                        ->back()
                        ->withErrors(['Remove the products first !']);
                    //--- Redirect Section Ends
                }
    ...
    {
        $datas = Category::orderBy('id', 'desc')->where('type', 'service')->get();
        //--- Integrating This Collection Into Datatables
        return Datatables::of($datas)
            ->addColumn('status', function (Category $data) {
                $class = $data->status == 1 ? 'drop-success' : 'drop-danger';
    ...
    {
        return view('admin.category.index2');
    }

    //*** GET Request
    public function create2()
    {
        return view('admin.category.create2');
    }
}
