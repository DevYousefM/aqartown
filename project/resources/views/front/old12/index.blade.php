...
@section('gsearch')
              ...
              <div class="img-area  " data-tilt data-tilt-glare data-tilt-max-glare="0.5">
                ...
                <div class="video-wrapper on-start">
                  <div class="play-btn-col-dir"><a class="video-link"
                      href="{{$gs->home_about_link}}" role="button"
        ...
        <div class="container">
          <div class="row   services-row">
            ...
            {{--    {{asset('assets/images/categories/'.$cat->photo)}}--}}
                <div class="back-face">
                  <div class="bg-img-wrapper"><img class="bg-img" src="assets/images/services/وحدات-التخزين.png"
                      alt="bg-img" /></div>
                  <div class="overlay-color"></div>
                  <div class="back-face-inner">
                    ...
                    <div class="desc-wrapper">
                      <p class="desc"> @if($langg->rtl == 1)
                          {{$cat->meta_description_ar}}
                        @else
                          {{$cat->meta_description}}
                        @endif </p>
                    </div>
                    <div class="btn-wrapper"><a class="btn-solid  " href="@if($langg->rtl == 1)
                      {{ route('front.category',['category' => $cat->slug_ar, 'lang' => $sign]) }}

                      @else
                      {{ route('front.category',['category' => $cat->slug, 'lang' => $sign]) }}

                      @endif">المزيد<i class="fas fa-arrow-right icon ">
                        </i></a></div>
                  </div>
                </div>
              </div>
 ...
 <section class="testimonials stack bg-img-section testimonials-3d dark-cards  d-flex align-items-center mega-section " id="testimonials-3d">
  <div class="particles-js  polygons" id="particles-js"></div>
  <div class="overlay-pattern-image-bg"></div>
  <div class="container">
    <div class="section-heading center-heading">
      <h2 class="section-title  wow fadeInUp" data-wow-delay=".2s">اراء <span class='hollow-text'>العملاء</span><span class="title-design-element "></span></h2>
      <div class="line line-on-center wow fadeIn" data-wow-delay=".7s"></div>
    </div>
            ...
            <div class="swiper-slide">
              <div class="testmonial-card d-flex align-items-center justify-content-center">
                <div class="testimonial-content">
                  <div class="customer-info "><img class="img-fluid " src="{{ $review->photo ? asset('assets/images/reviews/'.$review->photo):asset('assets/images/noimage.png') }}" alt="First Slide ">
                    <div class="customer-details">
                      <p class="customer-name">{{$langg->rtl == 1 ? $review->title_ar : $review->title}}</p>
                    {{--  <p class="customer-role">Lawyer</p>--}}
                    </div>
                  </div>
                  <div class="rating-stars"><i class="fas fa-star star-icon"></i><i class="fas fa-star star-icon"></i><i class="fas fa-star star-icon"></i><i class="fas fa-star star-icon"></i><i class="fas fa-star star-icon "></i></div>
