                    ...
                    <div class="container">
                        <div class="page-title-inner d-inline-block">
                            ...
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item"><a href="{{ route('front.blog',$sign) }}" title="">{{ $langg->lang18 }}</a></li>
                                <li class="breadcrumb-item active"> @if($langg->rtl == 1)
                                        {{$blog->title_ar}}
                                    @else
                                        {{$blog->title}}
                                    @endif </li>
                                ...
                                <div class="col-md-12 col-sm-12 col-lg-8">
                                    <div class="blog-detail w-100">
                                        <div class="blog-detail-info w-100">
                                            <img class="img-fluid w-100" src="{{ $blog->image ? asset('assets/images/blogs/'.$blog->image):asset('assets/images/noimage.png') }}" alt="Blog Detail Image">
                                            <h2 class="mb-0"> @if($langg->rtl == 1)
                                                    {{ $blog->title_ar}}
                                                @else
                                                    {{ $blog->title}}
                                                @endif</h2>
                                            <div class="post-meta2">
                                                <span class="post-date thm-clr">{{date('M d,Y', strtotime($blog->created_at))}}</span>
                                        ...
                                        <div class="detail-video position-relative w-100">
                                            <a class="d-inline-block position-absolute play-btn" data-fancybox href="{{ $blog->video }}" title="Video">
                                            ...
                                            <img class="img-fluid w-100" style="height: 360px;" src="{{ $blog->image ? asset('assets/images/blogs/'.$blog->image):asset('assets/images/noimage.png') }}" alt="Blog Detail Image 5">
                                        </div>
                                        @endif
                                            ...
                                            <div class="blog-detail-tags-wrap d-flex flex-wrap align-items-center w-100">
                                                ...
                                                <div class="tagclouds">
                                                    @foreach(explode(',', $blog->tags) as $key => $tag)
                                                        <a href="{{ route('front.blogtags',['slug' => $tag ,'lang' => $sign ]) }}" title="">  {{ $tag }}{{ $key != count(explode(',', $blog->tags)) - 1  ? '':''}}</a>
                                                ...
                                                <ul class="comments-list mb-0 list-unstyled">
                                                    ...
                                                    <li>
                                                        <div class="comment w-100">
                                                            <div class="comment-thumb rounded-circle"><img class="img-fluid rounded-circle" src="{{asset('assets/images/user-admin.png')}}" alt="Comment Image 1"></div>
                                                            <div class="comment-info">
                                                                <h4 class="mb-0">{{ $comment->name }}</h4>
                                                                <span class="thm-clr d-inline-block">{{ $comment->created_at->diffForHumans() }}</span>
                                        ...
                                        <div class="detail-form mt-60 detail-meta d-inline-block w-100">
                                            <h3>{{ $langg->lang40 }}</h3>
                                            <form class="w-100" id="comment-form" action="{{ route('product.comment') }}" method="POST">
                                                {{csrf_field()}}
                                                <input type="hidden" name="blog_id" id="blog_id" value="{{$blog->id}}">
                                                <div class="row mrg10">
                                                    <div class="col-md-6 col-sm-6 col-lg-6">
                                                        <input class="w-100" type="text" name="name" placeholder="{{ $langg->lang47 }}">
                                                    </div>
                                                    <div class="col-md-6 col-sm-6 col-lg-6">
                                                        <input class="w-100" type="email" name="email" placeholder="{{ $langg->lang49 }}">
                                                    </div>
                                                    <div class="col-md-6 col-sm-6 col-lg-6">
                                                        <input class="w-100" type="text" name="title" placeholder="{{ $langg->lang53 }}">
                                                    </div>
                                                    <div class="col-md-6 col-sm-6 col-lg-6">
                                                        <input class="w-100" type="tel" name="phone" placeholder="{{ $langg->lang48 }}">
                                                    </div>
                                                    <div class="col-md-12 col-sm-12 col-lg-12">
                                                        <textarea class="w-100" name="text" placeholder="{{ $langg->lang41 }}"></textarea>
                                                        <button class="thm-btn fill-btn submit-btn" type="submit">{{ $langg->lang46 }}<span></span></button>
                                                    </div>
                                                </div>
                                            </form>
                                        </div>
                                       ...
                                       {{-- <div class="widget-box w-100">
                                            ...
                                            <div class="articles-list w-100">
                                                ...
                                                <div class="article-box d-flex flex-wrap w-100">
                                                    ...
                                                    <div class="article-info">
                                                        <span class="post-date thm-clr">{{date('M d,Y', strtotime($blog->created_at))}}</span>
                                                        <h4 class="mb-0"><a href="{{route('front.blogshow',['id' => $blog->slug ,'lang' => $sign ])}}" title="">  @if($langg->rtl == 1)
                                                                    {{strlen($blog->title_ar) > 50 ? substr($blog->title_ar,0,50)."...":$blog->title_ar}}
                                                                @else
                                                                    {{strlen($blog->title) > 50 ? substr($blog->title,0,50)."...":$blog->title}}
                                                                @endif</a></h4>
                                        ...
                                        <div class="widget-box w-100">
                                            ...
                                            <ul class="cate-list mb-0 list-unstyled w-100">
                                                @foreach($bcats as $cat)
                                                    <li><a href="{{ route('front.blogcategory',['slug' => $cat->slug ,'lang' => $sign ]) }}" title="">
                                                            @if($langg->rtl == 1)
                                                                {{ $cat->name_ar }}
                                                            @else
                                                                {{ $cat->name }}
                                                            @endif </a><span>{{ $cat->blogs()->count() }}</span></li>
                                        ...
                                        <div class="widget-box w-100">
                                            ...
                                            <ul class="mb-0 list-unstyled w-100">
                                                @foreach($archives as $key => $archive)
                                                    <li><a href="{{ route('front.blogarchive',['slug' => $key ,'lang' => $sign ]) }}" title="">({{ count($archive) }}) - {{ $key }}</a></li>
                                        ...
                                        <div class="widget-box w-100">
                                            ...
                                            <div class="recent-events-list w-100">
                                                ...
                                                <div class="recent-event w-100">
                                                    <a href="{{ route('front.product', ['slug' => $data->slug , 'lang' => $sign]) }}" title=""><img class="img-fluid w-100" src="{{ $data->photo ? filter_var($data->photo, FILTER_VALIDATE_URL) ? $data->photo :asset('assets/images/products/'.$data->photo):asset('assets/images/noimage.png') }}" alt="Recent Image 1"></a>
                                                    <div class="recent-event-info">
                                                        <h4 class="mb-0"><a href="{{ route('front.product', ['slug' => $data->slug , 'lang' => $sign]) }}" title="">@if($langg->rtl == 1)
                                                                ...
                                                                @endif</a></h4>
                                                        <span class="thm-clr">{{$data->date}} - {{$data->location}}</span>
                                                    </div>
                                                </div>
                                                @endforeach
                                        ...
                                        <div class="widget-box w-100">
                                            ...
                                            <ul class="flickr-photos-list d-flex flex-wrap list-unstyled">
                                                ...
                                                @foreach($blogs as $b)
                                                <li><a href="javascript:void(0);" title=""><img class="img-fluid w-100" style="width: 80px ; height: 80px"  src="{{ $b->photo ? asset('assets/images/blogs/'.$b->photo):asset('assets/images/noimage.png') }}" alt="Flickr Image 1"></a></li>
                                                 @endforeach
        ...
        $.ajax({
            ...
            success:function(data)
            {
                ...
                $('#comment-form textarea').val('');
                  ...
                  $('.comments-list').prepend('<li>'+'<div class="comment w-100">'+'<div class="comment-thumb rounded-circle">'+
                     '<img class="img-fluid rounded-circle" src="{{asset('assets/images/user-admin.png')}}" alt="Comment Image 1">'+'</div>'
                    +'<div class="comment-info">'
