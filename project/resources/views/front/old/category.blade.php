                        ...
                        <div class="col-md-6 col-sm-6 col-lg-4">
                            <div class="event-grid-box mb-30 w-100">
                                <div class="event-grid-img w-100 overflow-hidden position-relative">
                                    <img class="img-fluid w-100" src="{{ $productt->photo ? filter_var($productt->photo, FILTER_VALIDATE_URL) ? $productt->photo :asset('assets/images/products/'.$productt->photo):asset('assets/images/noimage.png') }}" alt="Event Image 1">
                                    <span class="position-absolute"><a class="rounded-circle" href="javascript:void(0);" title=""><i class="fas fa-heart"></i></a></span>
                                    <a class="thm-btn fill-btn" href="{{ route('front.product', ['slug' => $productt->slug_ar , 'lang' => $sign]) }}" title="">Book Now<span></span></a>
                                </div>
                                <div class="event-grid-info w-100">
                                    <h3 class="mb-0"><a href="{{ route('front.product', ['slug' => $productt->slug_ar , 'lang' => $sign]) }}" title="">@if($langg->rtl == 1)
                                            ...
                                            @endif</a></h3>
                                    @if(!empty($productt->date))    <span class="event-date thm-clr d-block">{{$productt->date}}</span>@endif
                                    <ul class="event-grid-meta mb-0 list-unstyled d-flex flex-wrap">
                                        @if(!empty($productt->time))     <li><i class="far fa-clock"></i>{{$productt->time}}</li>@endif
                                            @if(!empty($productt->price_from))      <li><i class="fas fa-tags"></i>{{$productt->price_from}}</li>@endif
                                    </ul>
                                    @if(!empty($productt->location))      <span class="event-loc d-block"><i class="fas fa-map-pin"></i>{{$productt->location}}</span>@endif
                                </div>
                            </div>
                        </div>
                        @endforeach
