...
display: grid;">
  ...
  padding: 20px;
                        ...
                        <img src="{{asset('assets/images/categories/'.$category->photo)}}" style="width: 100%;
height: 278px;
border-radius: 30px;
object-fit: cover;">
...
color: #fff;
border-radius: 42px;
background-color: #0053A6;box-shadow: 0 6px 8px rgb(0 0 0 / 10%);cursor: pointer;" onclick="window.location.href='@if($langg->rtl == 1)
                                                {{ route('front.category',['category' => $category->slug_ar, 'lang' => $sign]) }}
                                                  @else
                                                {{ route('front.category',['category' => $category->slug, 'lang' => $sign]) }}
                                            @endif'">
