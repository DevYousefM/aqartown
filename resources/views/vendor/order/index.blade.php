                                        ...
                                        @foreach ($orderr as $order)
                                            @php
                                                ...
                                                if ($user->shipping_cost != 0) {
                                                    $price += round(
                                                        $user->shipping_cost * $order->order->currency_value,
                                                        2,
                                                    );
                                                }
                                                if (
                                                ...
                                                <td>
                                                    ...
                                                    <div class="action-list">
                                                        ...
                                                        <select class="vendor-btn {{ $order->status }}">
                                                            <option
                                                                value="{{ route('vendor-order-status', ['id1' => $order->order->order_number, 'status' => 'pending']) }}"
                                                                {{ $order->status == 'pending' ? 'selected' : '' }}>
                                                                {{ $langg->lang540 }}</option>
                                                            <option
                                                                value="{{ route('vendor-order-status', ['id1' => $order->order->order_number, 'status' => 'processing']) }}"
                                                                {{ $order->status == 'processing' ? 'selected' : '' }}>
                                                                {{ $langg->lang541 }}</option>
                                                            <option
                                                                value="{{ route('vendor-order-status', ['id1' => $order->order->order_number, 'status' => 'completed']) }}"
                                                                {{ $order->status == 'completed' ? 'selected' : '' }}>
                                                                {{ $langg->lang542 }}</option>
                                                            <option
                                                                value="{{ route('vendor-order-status', ['id1' => $order->order->order_number, 'status' => 'declined']) }}"
