    ...
    <div class="col-lg-5">
        ...
        <div class="xzoom-container">
            <img class="quick-zoom" id="xzoom-magnific1"
                src="{{ filter_var($product->photo, FILTER_VALIDATE_URL) ? $product->photo : asset(access_public() . 'assets/images/products/' . $product->photo) }}"
                xoriginal="{{ filter_var($product->photo, FILTER_VALIDATE_URL) ? $product->photo : asset(access_public() . 'assets/images/products/' . $product->photo) }}" />
            <div class="xzoom-thumbs">

                <div class="quick-all-slider">

                    <a
                        href="{{ filter_var($product->photo, FILTER_VALIDATE_URL) ? $product->photo : asset(access_public() . 'assets/images/products/' . $product->photo) }}">
                    ...
                    @foreach ($product->galleries as $gal)
                        <a href="{{ asset(access_public() . 'assets/images/galleries/' . $gal->photo) }}">
                            <img class="quick-zoom-gallery" width="80"
        ...
        <div class="right-area">
            <div class="product-info">
                ...
                @php
                    ...
                    $sizes = App\Models\Color::where('product_id', $product->id)
                        ->where('size_qty', '>', 0)
                        ->distinct('size')
                        ->get();
                ...
                @if (!empty($product->size))
                    <div class="product-size">
                        ...
                        <ul class="siz-list siz-lists">
                            @php
                                $co = App\Models\Color::where('product_id', $product->id)
                                    ->where('size', $product->size[0])
                                    ->where('size_qty', '>', 0)
                                    ->get();
                                $is_first = true;
                            @endphp
                ...
                <div class="info-meta-3">
                    <ul class="meta-list">
                        ...
                        @if ($product->product_type == 'affiliate')
                            ...
                            <li class="addtocart">
                                <a href="{{ route('affiliate.product', ['slug' => $product->slug, 'lang' => $sign]) }}"
                        ...
                        @else
                            ...
                            @else
                                ...
                                <li class="addtocart ">
                                    <a
                                        href="{{ route('product.cart.quickadd', ['id' => $product->id, 'lang' => $sign]) }}"><i
...
<script src="{{ asset(access_public() . 'assets/front/js/quicksetup.js') }}"></script>
