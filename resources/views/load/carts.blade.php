    ...
    <div class="dropdownmenu-wrapper sss">
        @if (Session::has('cart'))
                                        ...
                                        @if (!$slang)
                                            @if ($lang->id == 2)
                                                {!! strlen($product['item']['name_ar']) > 100
                                                    ? substr($product['item']['name_ar'], 0, 100) . '...'
                        ...
                        <figure class="product-image-container">
                            <a href="{{ route('front.product', ['slug' => $product['item']['slug'], 'lang' => $sign]) }}"
                                ...
                                <img src="{{ $product['item']['photo'] ? (filter_var($product['item']['photo'], FILTER_VALIDATE_URL) ? $product['item']['photo'] : asset(access_public() . 'assets/images/products/' . $product['item']['photo'])) : asset(access_public() . 'assets/images/noimage.png') }}"
                                    style="width:100px;height:auto;" alt="product">
                            </a>
                            <a class="cart-remove btn-remove"
            ...
            <div class="dropdown-cart-total">
                ...
                <span
                    class="cart-total-price">{{ Session::has('cart') ? App\Models\Product::convertPrice(Session::get('cart')->totalPrice) : '0.00' }}</span>
            </div><!-- End .dropdown-cart-total -->

            <div class="dropdown-cart-action">
                <a href="{{ url('carts') }}" class="btn">{{ $langg->lang5 }}</a>
                &nbsp; &nbsp;

                <a href="{{ url('checkout') }}" class="btn btn-block">{{ $langg->lang7 }}</a>
